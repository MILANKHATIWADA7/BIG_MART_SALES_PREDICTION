...
def result():
    if request.method == 'POST':
        try:
            # Attempt to convert input fields to float
            ...
            if errors:
                ...
                try:
                    model_path = r'C:\Users\Milan\Desktop\Big-Mart-Sales-Prediction-System\linear_regression2.sav'
                    ...
                    return render_template('sales_prediction.html', prediction=float(Y_pred))

                except Exception as e:
                        return render_template('error.html', error=str(e))
        except ValueError:
            # Flash a message if any of the fields are empty
            if not all(request.form.get(field) for field in ['item_weight', 'item_fat_content', 'item_visibility', 'item_type', 'item_mrp', 'outlet_establishment_year', 'outlet_size', 'outlet_location_type', 'outlet_type']):
                flash("One or more input fields are empty. Please fill in all fields.")
...
def profile():
    if 'email' in session:
        ...
        if request.method == 'POST':
            ...
            if user and user[3] == old_password:  # Assuming the password is stored in the fourth column
        ...
        flash('Please login to access profile page', 'warning')
        return redirect(url_for('default'))
